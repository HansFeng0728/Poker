...
public class RedisUtil {
	...
	public <T> T hashGet(String key, String field, Class<T> classType) {
		String result = hashGet(key, field);
		T fromJson = JsonUtil.fromJson(result, classType);
		return fromJson;
	}
	...
	public void listDel(String key, Object val) {
		Jedis jedis = null;
		try {
			jedis = pool.getResource();
			jedis.lrem(key, -1, JsonUtil.toJson(val));
		} catch (Exception e) {
			logger.error("", e);
		} finally {
			returnJedis(jedis);
